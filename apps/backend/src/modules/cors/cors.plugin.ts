...
import { ENV, type PublicEnv } from "../env";

export type Origin = string | RegExp;

export interface CORSConfig {
	origin?: boolean | ((app_env: PublicEnv["NODE_ENV"]) => Origin[]);
...
export const cors = (config?: CORSConfig) => {
	...
	const handleOrigin = (set: Context["set"], request: Request, app_env: PublicEnv["NODE_ENV"]) => {
		if (origin === false) {
			return;
		}
	...
	return app
		...
		.onRequest(({ set, request }) => {
			handleOrigin(set, request, ENV.NODE_ENV);
